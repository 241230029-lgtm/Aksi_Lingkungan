            ...
            <div class="p-10">
                ...
                <div class="mt-10 border-t border-gray-100 pt-8">
                    ...
                    @elseif($item->kategori == 'Eco-Volunteer')
                        ...
                        @else
                            @if(session('success'))
                                <div class="mb-4 p-4 bg-green-50 text-green-700 border border-green-200 rounded-xl font-semibold">{{ session('success') }}</div>
                            @endif
                            @if(session('error'))
                                <div class="mb-4 p-4 bg-red-50 text-red-700 border border-red-200 rounded-xl font-semibold">{{ session('error') }}</div>
                            @endif
                            ...
                            <form method="POST" action="{{ route('volunteer.daftar', $item->id_kegiatan) }}">
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea name="alasan_bergabung" rows="4" required placeholder="Ceritakan motivasi Anda..." class="w-full border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-green-500 outline-none"></textarea>
                                </div>
                                <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-xl font-bold transition shadow">Kirim Pendaftaran</button>
                            </form>
                        @endif
