...
<div class="grid lg:grid-cols-3 md:grid-cols-2 gap-8">
    ...
    <div class="bg-white rounded-2xl shadow hover:shadow-xl transition overflow-hidden">
        ...
        <div class="p-6">
            ...
            <p class="text-gray-500 mt-3">{{ Str::limit($info->deskripsi, 100) }}</p>
            <div class="flex justify-between items-center mt-6">
                <div>
                    <p class="text-sm text-gray-500">{{ $info->user->name ?? 'Admin' }}</p>
                    <p class="text-sm text-gray-500">{{ $info->created_at->format('d M Y') }}</p>
                </div>
                <a href="{{ route('information.show', $info->id_kegiatan) }}" class="bg-green-600 text-white px-5 py-2 rounded-xl hover:bg-green-700">Baca</a>
            </div>
        </div>
    </div>
    @endforeach
