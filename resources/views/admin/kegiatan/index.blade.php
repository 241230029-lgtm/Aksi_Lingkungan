...
<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <button onclick="openModal('add')" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm px-5 py-2.5 rounded-xl transition shadow-sm cursor-pointer">+ Tambah</button>
    </div>
</div>
                ...
                <tr class="hover:bg-gray-50/40 transition">
                    ...
                    <td class="p-4 px-6">
                        <div class="flex items-center justify-center gap-2">
                            <button onclick="openModal('edit', {{ $item->id_kegiatan ?? $item->id }})" class="p-2 rounded-lg transition cursor-pointer bg-amber-50 hover:bg-amber-100 text-amber-700" title="Edit">
                            ...
                            <button onclick="deleteItem({{ $item->id }})" class="p-2 rounded-lg transition cursor-pointer bg-red-50 hover:bg-red-100 text-red-600" title="Hapus">
...
<div id="modal" style="display: none; position: fixed; inset: 0; z-index: 50; background-color: rgba(0,0,0,0.5); align-items: center; justify-content: center; padding: 1rem; overflow-y: auto;">
    <div style="background-color: white; border-radius: 1rem; max-width: 32rem; width: 100%; padding: 1.5rem; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1); max-height: 90vh; overflow-y: auto; margin: auto;">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem; padding-bottom: 0.5rem; border-bottom: 1px solid #f3f4f6;">
            <h3 id="modalTitle" style="font-size: 1.25rem; font-weight: 700; color: #111827;"></h3>
        ...
        <form id="kegiatanForm" method="POST" enctype="multipart/form-data">
            ...
            <div style="margin-top: 1.5rem; display: flex; justify-content: flex-end; gap: 0.75rem; border-top: 1px solid #f3f4f6; padding-top: 1rem;">
                ...
                <button type="submit" id="submitBtn" class="px-4 py-2 text-sm font-semibold text-white rounded-xl cursor-pointer transition">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function openModal(mode, id = null) {
        document.getElementById('modalTitle').textContent = mode === 'add' ? 'Tambah Kegiatan' : 'Edit Kegiatan';
        const submitBtn = document.getElementById('submitBtn');

        if (mode === 'add') {
            submitBtn.style.backgroundColor = '#2563eb';
            submitBtn.onmouseover = function() { this.style.backgroundColor = '#1d4ed8'; };
            submitBtn.onmouseout = function() { this.style.backgroundColor = '#2563eb'; };
            ...
            submitBtn.onmouseover = function() { this.style.backgroundColor = '#d97706'; };
            submitBtn.onmouseout = function() { this.style.backgroundColor = '#f59e0b'; };
